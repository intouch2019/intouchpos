												...
												<div class="col-sm-6 col-md-6 col-lg-6 col-xl-4 col-xxl-3 product-item" data-category="<?php echo $product['category_id']; ?>" data-sku="<?php echo htmlspecialchars($product['sku']); ?>" style="display: block;">
													<div class="product-info card mb-0<?php echo ($product['total_stock'] <= 0) ? ' out-of-stock' : ''; ?>">
														...
														<div class="d-flex flex-column align-items-start w-100">
                                                                                                                    ...
                                                                                                                    <span class="badge <?php echo ($product['total_stock'] > 5) ? 'bg-success' : (($product['total_stock'] > 0) ? 'bg-warning text-dark' : 'bg-danger'); ?>">
                                                                                                                        Available Stock: <?php echo $product['total_stock']; ?>
																...
																<input type="text" class="form-control text-center product-qty" name="qty" value="1" min="1" max="<?php echo $product['total_stock']; ?>">
														...
														<?php if ($product['total_stock'] <= $product['min_stock_level']): ?>
                                                <!--<div class="stock-warning">
															<small class="text-danger">Low Stock: <?php // echo $product['total_stock']; ?> left</small>
														</div>-->
														<?php endif; ?>
													</div>
												</div>
...
//        function addToCart(productId, productName, price, batch = null) {
        function addToCart(productId, productName, price, batch = null) {
            ...
            if (exchangeChecked) {
                addExchangeToCart(productId, productName, price, batch);
                return;
            }
...
//            alert(price);
    ...
    function addExchangeToCart(productId, productName, price, batch = null) {
        ...
        fetch('cart_api.php', {
            ...
            body: `action=add_exchange&product_id=${productId}&quantity=1&price=${newItemTotal}&batch=${batch}&name=${encodeURIComponent(productName)} (Exchange)`
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="qty-item m-0">
                                    <a href="javascript:void(0);" class="dec d-flex justify-content-center align-items-center" data-bs-toggle="tooltip" data-bs-placement="top" title="minus" onclick="decreaseCartQuantity(${item.id}, '${item.batch_code}')"><i class="ti ti-minus"></i></a>
                                    <input type="text" class="form-control text-center" name="qty" value="${item.quantity}" readonly>
                                    <a href="javascript:void(0);" class="inc d-flex justify-content-center align-items-center" data-bs-toggle="tooltip" data-bs-placement="top" title="plus" onclick="increaseCartQuantity(${item.id}, '${item.batch_code}')"><i class="ti ti-plus"></i></a>
                                </div>
                            </td>
        ...
        function removeFromCart(productId, batch) {
            ...
            fetch('cart_api.php', {
                ...
                body: `action=${action}&product_id=${productId}&batch=${batch}`
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
        ...
        function increaseCartQuantity(productId, batch) {
            const item = cart.find(item => item.id === productId);
            if (item) {
                updateCartQuantity(productId, item.quantity + 1, batch);
            }
        }
        
        // Function to decrease cart item quantity
        function decreaseCartQuantity(productId,batch) {
            const item = cart.find(item => item.id === productId);
            if (item) {
                if (item.quantity > 1) {
                    updateCartQuantity(productId, item.quantity - 1, batch);
                } else {
                    removeFromCart(productId,batch);
                }
            }
        }
        
        // Function to update cart quantity
        function updateCartQuantity(productId, quantity, batch) {
            fetch('cart_api.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: `action=update&product_id=${productId}&quantity=${quantity}&batch=${batch}`
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
...
function addItemToCart(productId, productName, price, quantity, batch = NULL) {
    const exchangeCheckbox = document.getElementById('exchangeCheckbox');
    const isExchange = exchangeCheckbox.checked;
    
    let action = 'add';
    let body = `action=${action}&product_id=${productId}&quantity=${quantity}&batch=${batch}`;

    if (isExchange) {
        action = 'add_exchange';
        const finalPrice = -Math.abs(price);
        body = `action=${action}&product_id=${productId}&quantity=${quantity}&price=${finalPrice}&name=${productName} (Exchange)&batch=${batch}`;
    }
...
// Function to view online order products inside the card
function viewOnlineOrderProducts(orderId) {
    console.log('Viewing products for order:', orderId);
    ...
    if (!orderCard) return;
        ...
        let existingProducts = orderCard.querySelector(".products-container");
        if (existingProducts) {
            existingProducts.classList.toggle("d-none");
            return;
        }

        // Show loading state
        let loadingHtml = `
        <div class="products-container mt-3 p-3 bg-light rounded text-center shadow-sm">
        <i class="ti ti-loader ti-spin text-primary fs-4"></i>
        <p class="text-muted small mt-2">Loading products...</p>
        </div>`;
        orderCard.insertAdjacentHTML("beforeend", loadingHtml);

        // Fetch products
        fetch('get_online_orders_items.php?order_id=' + orderId)
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok, status ' + response.status);
            }
    ...
    .then(data => {//
        const container = orderCard.querySelector(".products-container");
        if (!data.items || data.length === 0) {
            container.innerHTML = `
            <div class="text-center text-muted">
            <i class="ti ti-package-off fs-4"></i>
            <p class="mt-1 small">No products found for this order.</p>
            </div>`;
            return;
        }
        ...
        // Build products HTML
        let productsHtml = `
        <div class="products-container mt-3 p-3 bg-white rounded shadow-sm border">
        <div class="d-flex justify-content-between align-items-center mb-3">
        <h6 class="fw-bold mb-0">
        <i class="ti ti-shopping-bag me-2 text-primary"></i>
        Products in this Order
        </h6>
        <button class="btn btn-sm btn-outline-danger" onclick="this.closest('.products-container').classList.add('d-none')">
        ...
        data.items.forEach(p => {
            ...
            <div class="d-flex align-items-center p-2 border rounded bg-light h-100">
            <img src="assets/img/products/${p.image || 'images(1).jpg'}" 
            alt="${p.name || ''}" 
            class="rounded me-3" 
            style="width:50px;height:50px;object-fit:cover;">
            <div class="flex-grow-1">
            <h6 class="mb-1">${p.name || ''}</h6>
            <small class="text-muted">Qty: ${p.qty || 0}</small>
            </div>
            <p class="mb-0 fw-bold text-success ms-3">
            ₹${p.price !== undefined ? parseFloat(p.price).toFixed(2) : '0.00'}
            </p>
            </div>
            </div>`;
        });
        ...
        container.outerHTML = productsHtml;
    })
    .catch(err => {
        console.error('Error loading order products:', err);
        orderCard.querySelector(".products-container").innerHTML = `
        <div class="text-center text-danger small">
        <i class="ti ti-alert-circle me-1"></i> Failed to load products: ${err.message}
        </div>`;
    });
}
...
function filterOnlineOrders() {
  let input = document.getElementById("onlineOrderSearch").value.toLowerCase();
  let orders = document.querySelectorAll("#online-orders > div");
