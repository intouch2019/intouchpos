  ...
  .then(data => {
      ...
      if (data.success && data.orders && data.orders.length > 0) {
        data.orders.forEach(order => {
          ...
          const reviveButton = `
          <button class="btn btn-success btn-sm px-3" onclick="reviveOrder(${order.id})">
          <i class="ti ti-refresh me-1"></i> Revive Order
          </button>`;

          container.innerHTML += `
          <div class="card shadow-sm border-0 mb-3 rounded-3 order-card">
          ...
          <button class="btn btn-teal btn-sm px-3" onclick="viewOrderProducts(${order.id})">
