...
try {
    ...
    // Mark order as revived instead of deleting (only if not already revived)
    $update_sql = "UPDATE orders SET hold_reference = CASE 
                   WHEN hold_reference NOT LIKE '%_REVIVED' THEN CONCAT(hold_reference, '_REVIVED')
                   ELSE hold_reference 
                   END 
                   WHERE id = ?";
