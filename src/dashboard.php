...
// Total products
$prod_count_sql = "SELECT COUNT(*) as count FROM products WHERE is_active = 1";
$prod_result = mysqli_query($link, $prod_count_sql);
if ($prod_result && $row = mysqli_fetch_assoc($prod_result)) {
    $stats['total_products'] = $row['count'];
}

// Total customers
$cust_count_sql = "SELECT COUNT(*) as count FROM customers WHERE is_active = 1";
$cust_result = mysqli_query($link, $cust_count_sql);
...
$today_result = mysqli_query($link, $today_orders_sql);
if ($today_result && $row = mysqli_fetch_assoc($today_result)) {
    $stats['today_orders'] = $row['count'];
}

// Total sales today
$today_sales_sql = "SELECT COALESCE(SUM(total_amount), 0) as total FROM orders WHERE DATE(created_at) = CURDATE() AND payment_status = 'completed'";
$sales_result = mysqli_query($link, $today_sales_sql);
                ...
                <div class="col-xxl-4 col-xl-12 d-flex">
                    <div class="card flex-fill">
                        <div class="card-header pb-2 d-flex align-items-center justify-content-between flex-wrap">
                            ...
                            <a href="purchase-transaction.php" class="btn btn-light btn-sm mb-2">View All</a>
                        </div>
